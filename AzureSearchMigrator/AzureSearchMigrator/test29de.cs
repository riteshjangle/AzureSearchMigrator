    ...
    class Program
    {
            ...
            int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            try
            {
                ...
                int consecutiveSameCountBatches = 0;
                int previousBatchCount = 0;
                ...
                while (hasMoreDocuments && totalMigratedDocuments < totalDocuments)
                {
                    ...
                    try
                    {
                        ...
                        if (documents == null || documents.Count == 0)
                        {
                            ...
                            // Try with skip as fallback
                            if (consecutiveEmptyBatches >= 2)
                            {
                                Console.WriteLine("Trying skip-based pagination as fallback...");
                                documents = await GetDocumentsWithSkipAsync(
                                    sourceServiceName, sourceIndexName, sourceApiKey,
                                    batchSize, totalMigratedDocuments, apiVersion);
                                
                                if (documents != null && documents.Count > 0)
                                {
                                    consecutiveEmptyBatches = 0;
                                    Console.WriteLine($"Skip-based pagination retrieved {documents.Count} documents");
                                }
                            }
                            continue;
                        }
                        ...
                        // Check for stuck pagination (same document count multiple times)
                        if (documents.Count == previousBatchCount && documents.Count > 0)
                        {
                            consecutiveSameCountBatches++;
                            Console.WriteLine($"Warning: Same batch count ({documents.Count}) for {consecutiveSameCountBatches} consecutive batches");
                            
                            if (consecutiveSameCountBatches >= 3)
                            {
                                Console.WriteLine("Appears to be stuck in pagination - advancing last key manually");
                                // Force advance by getting the last key and incrementing
                                ...
                                if (lastDocument.TryGetValue(keyFieldName, out var keyValue) && keyValue != null)
                                {
                                    ...
                                    // Add a small increment if it's numeric, or use a different strategy for strings
                                    if (long.TryParse(lastKey, out long longKey))
                                    {
                                        lastKey = (longKey + 1).ToString();
                                    }
                                    else if (double.TryParse(lastKey, out double doubleKey))
                                    {
                                        lastKey = (doubleKey + 1).ToString();
                                    }
                                    Console.WriteLine($"Manually advanced last key to: {lastKey}");
                                }
                                consecutiveSameCountBatches = 0;
                            }
                        }
                        else
                        {
                            consecutiveSameCountBatches = 0;
                            previousBatchCount = documents.Count;
                        }
                        ...
                        if (documents.Count > 0)
                        {
                            ...
                            {
                                lastKey = keyValue.ToString();
                                Console.WriteLine($"Last key in batch {batchNumber}: {lastKey}");
                            }
                            else
                            {
                                Console.WriteLine($"Warning: Could not get key value from last document in batch {batchNumber}");
                                // Fallback to skip-based pagination
                                lastKey = null;
                            }
                        }
                        ...
                        // Check if we got fewer documents than requested - this doesn't necessarily mean end of data
                        if (documents.Count < batchSize)
                        {
                            Console.WriteLine($"Got fewer documents ({documents.Count}) than batch size ({batchSize}) - checking if this is really the end...");
                            
                            // Double-check by trying to get one more document after the last key
                            if (!string.IsNullOrEmpty(lastKey))
                            {
                                var testDocuments = await GetDocumentsWithCursorAsync(
                                    sourceServiceName, sourceIndexName, sourceApiKey,
                                    1, keyFieldName, lastKey, apiVersion);

                                if (testDocuments == null || testDocuments.Count == 0)
                                {
                                    Console.WriteLine("Confirmed end of data - no more documents available");
                                    hasMoreDocuments = false;
                                }
                                else
                                {
                                    Console.WriteLine($"Found {testDocuments.Count} more documents after last key - continuing migration");
                                }
                            }
                            else
                            {
                                // If we can't use cursor, assume end of data when we get fewer documents
                                Console.WriteLine("Assuming end of data due to fewer documents than batch size");
                                hasMoreDocuments = false;
                            }
                        }
                ...
                if (totalMigratedDocuments < totalDocuments)
                {
                    ...
                    // Try one final attempt with skip-based pagination for remaining documents
                    if (totalMigratedDocuments < totalDocuments)
                    {
                        Console.WriteLine("Attempting to migrate remaining documents using skip-based pagination...");
                        await MigrateRemainingWithSkipAsync(
                            sourceServiceName, sourceIndexName, sourceApiKey,
                            targetServiceName, targetIndexName, targetApiKey,
                            batchSize, totalMigratedDocuments, totalDocuments, apiVersion);
                    }
                }
            }
            catch (Exception ex)
            ...
            int top, string keyFieldName, string lastKey, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (!string.IsNullOrEmpty(lastKey))
                    {
                        // Use proper OData filter syntax - try different approaches
                        try
                        {
                            // First try with simple greater than
                            filter = $"{keyFieldName} gt '{EscapeODataString(lastKey)}'";
                            
                            // For string keys, we might need to handle case sensitivity
                            if (IsLikelyStringKey(lastKey))
                            {
                                // Also try with greater than or equal and not equal to handle duplicates
                                filter = $"{keyFieldName} gt '{EscapeODataString(lastKey)}'";
                            }
                        }
                        catch (Exception ex)
                    ...
                    for (int retry = 0; retry < 3; retry++)
                    {
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                if (documents.Count > 0)
                                {
                                    var firstKey = documents.First().GetValueOrDefault(keyFieldName)?.ToString() ?? "null";
                                    var lastKeyValue = documents.Last().GetValueOrDefault(keyFieldName)?.ToString() ?? "null";
                                    Console.WriteLine($"Retrieved {documents.Count} documents, first key: {firstKey}, last key: {lastKeyValue}");
                                }
                            ...
                            else
                            {
                                ...
                                Console.WriteLine($"Failed to get documents (attempt {retry + 1}): {response.StatusCode}, Error: {errorContent}");

                                // If it's a bad request, try without filter
                        ...
                        {
                            Console.WriteLine($"Error getting documents (attempt {retry + 1}): {ex.Message}");
                            if (retry < 2)
                            {
                                await Task.Delay(2000 * (retry + 1));
                            }
            ...
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        Console.WriteLine($"Skip-based pagination retrieved {documents.Count} documents (skip: {skip})");
                        return documents;
                    }
                    else
                    {
                        var errorContent = await response.Content.ReadAsStringAsync();
                        Console.WriteLine($"Failed to get documents with skip: {response.StatusCode}, Error: {errorContent}");
                    }
                }
            }
            catch (Exception ex)
        ...
        static async Task MigrateRemainingWithSkipAsync(
            string sourceServiceName, string sourceIndexName, string sourceApiKey,
            string targetServiceName, string targetIndexName, string targetApiKey,
            int batchSize, int alreadyMigrated, int totalDocuments, string apiVersion)
        {
            int remaining = totalDocuments - alreadyMigrated;
            if (remaining <= 0) return;

            Console.WriteLine($"Migrating remaining {remaining} documents using skip-based pagination...");
            ...
            {
                batchNumber++;
                int currentBatchSize = Math.Min(batchSize, totalDocuments - skip);

                try
                {
                    var documents = await GetDocumentsWithSkipAsync(
                        sourceServiceName, sourceIndexName, sourceApiKey,
                        currentBatchSize, skip, apiVersion);
                    ...
                    if (documents != null && documents.Count > 0)
                    {
                        ...
                        if (success)
                        {
                            ...
                            Console.WriteLine($"✓ Skip Batch {batchNumber}: {documents.Count} docs - Total migrated: {skip:N0}/{totalDocuments:N0}");
                            
                            if (documents.Count < currentBatchSize)
                            {
                                Console.WriteLine("Reached end of data with skip-based pagination");
                                break;
                            }
                        }
                        else
                    ...
                    {
                        Console.WriteLine("No documents returned with skip-based pagination - stopping");
                        break;
                    }
                ...
                {
                    Console.WriteLine($"Error in skip batch {batchNumber}: {ex.Message}");
                    break;
                }

                // Safety limit
                if (batchNumber > 100) break;
            }

            Console.WriteLine($"Skip-based migration completed. Final count: {skip:N0}/{totalDocuments:N0}");
        }

        static bool IsLikelyStringKey(string key)
        {
            // Check if the key is likely a string (not purely numeric)
            return !long.TryParse(key, out _) && !double.TryParse(key, out _);
        }
        ...
        static string EscapeODataString(string value)
        {
            ...
            // For OData filters, we generally don't need to escape other characters
            // as the string will be properly quoted in the filter
            return escaped;
        }
