    ...
    class Program
    {
            ...
            int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            try
            {
                ...
                while (hasMoreDocuments && totalMigratedDocuments < totalDocuments && currentSkip < totalDocuments)
                {
                    batchNumber++;

                    try
                    {
                        // Get documents using REST API with skip/top
                        var documents = await GetDocumentsViaApiAsync(
                            sourceServiceName, sourceIndexName, sourceApiKey,
                            currentSkip, batchSize, keyFieldName, apiVersion);
                        ...
                        else
                        {
                            ...
                            // Still advance skip to continue
                            currentSkip += documents.Count;
                        }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // Try to continue with next batch after a delay
        ...
        static async Task<string> GetKeyFieldNameViaApiAsync(string serviceName, string indexName, string apiKey, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    // Get a single document to inspect fields
                    var url = $"https://{serviceName}.search.windows.net/indexes/{indexName}/docs/search?api-version={apiVersion}&$top=1";
                    httpClient.DefaultRequestHeaders.Add("api-key", apiKey);
                        ...
                        using (var doc = JsonDocument.Parse(json))
                        {
                            if (doc.RootElement.TryGetProperty("value", out var valueArray) && valueArray.GetArrayLength() > 0)
                            {
                                ...
                                foreach (var prop in firstDoc.EnumerateObject())
                                {
                                    ...
                                    if (prop.Name.ToLower() == "id" || prop.Name.ToLower() == "key" ||
                                        prop.Name.ToLower().Contains("id") || prop.Name.ToLower().Contains("key"))
                                    {
                                        return prop.Name;
                                    }
                                }
                                // Return first field if no obvious key found
        ...
        static async Task<List<JsonElement>> GetDocumentsViaApiAsync(
            string serviceName, string indexName, string apiKey,
            int skip, int top, string keyFieldName, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
        ...
        static async Task<bool> IndexDocumentsViaApiAsync(
            string serviceName, string indexName, string apiKey,
            List<JsonElement> documents, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
