    ...
    class Program
    {
            ...
            int totalDocuments)
        {
            ...
            try
            {
                ...
                // Use search continuation with proper async enumeration
                var options = new SearchOptions
                {
                    Size = batchSize
                };

                // Get all results using proper async pagination
                SearchResults<SearchDocument> results = await sourceClient.SearchAsync<SearchDocument>("*", options);
                
                // Process all pages correctly
                        ...
                        try
                        {
                            ...
                            foreach (var result in indexResult.Results)
                            {
                                if (!result.Succeeded)
                                {
                                    batchSuccess = false;
                                    failedInBatch++;
                                    if (retryCount == maxRetries - 1) // Last retry
                                    {
                                        Console.WriteLine($"  Failed document: Key={result.Key}, Error={result.ErrorMessage}");
                                    }
                                }
                            }
                ...
                if (totalMigratedDocuments < totalDocuments)
                {
                    ...
                    Console.WriteLine("This might indicate a pagination issue. Consider using a different approach.");
                }
            }
            catch (Exception ex)
            {
