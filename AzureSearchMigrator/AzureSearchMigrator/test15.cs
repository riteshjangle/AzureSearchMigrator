    ...
    class test
    {
        ...
        static async Task<int> GetTotalDocumentCountAsync(SearchClient sourceClient)
        {
            try
            {
                ...
                // Get the total count directly from the results
        ...
        static async Task MigrateDataWithPagingAsync(
            ...
            try
            {
                ...
                await foreach (Page<SearchResult<SearchDocument>> page in results.GetResultsAsync().AsPages())
                {
                    ...
                    try
                    {
                        ...
                        bool allSucceeded = indexResult.Results.All(r => r.Succeeded);
                        ...
                        if (allSucceeded)
                        {
                            ...
                            Console.WriteLine(
                                $"✓ Batch {successfulBatches + failedBatches}: {batch.Count} docs - " +
                                $"Total: {totalMigratedDocuments:N0}/{totalDocuments:N0} ({percentage:F1}%)");
                        }
                        else
                        {
                            failedBatches++;
                            int errors = indexResult.Results.Count(r => !r.Succeeded);
                            Console.WriteLine(
                                $"✗ Batch {successfulBatches + failedBatches}: {errors} errors in {batch.Count} docs");
                        }
                    }
                    catch (Exception ex)
                    {
