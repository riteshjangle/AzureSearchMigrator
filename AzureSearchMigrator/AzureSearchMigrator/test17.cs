    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                await MigrateAllDocumentsWithOrderByAsync(sourceClient, targetClient, batchSize, totalDocuments);
        ...
        static async Task MigrateAllDocumentsWithOrderByAsync(
            ...
            int totalDocuments)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("ERROR: Could not determine key field name. Using default 'key'.");
                    keyFieldName = "key";
                }

                Console.WriteLine($"Using key field for ordering: {keyFieldName}");
                ...
                while (hasMoreDocuments && totalMigratedDocuments < totalDocuments)
                {
                    ...
                    var options = new SearchOptions
                    {
                        ...
                        OrderBy = { $"{keyFieldName} asc" } // Order by key to ensure consistent pagination
                    };
        ...
        static async Task<string> GetKeyFieldNameAsync(SearchClient client)
        {
            try
            {
                // Get a single document to inspect its fields
                var options = new SearchOptions
                {
                    Size = 1,
                    Select = { "*" }
                ...
                await foreach (var result in results.GetResultsAsync())
                {
                    ...
                    var possibleKeys = new[] { "id", "key", "documentId", "docId", "Id", "Key" };
                    ...
                    // If no common key found, return the first field
                    return doc.Keys.FirstOrDefault() ?? "key";
                }
