...
namespace AzureSearchMigrator
{
    class Program
    {
           ...
           int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            // Get the unique document key name for tie-breaking
            string uniqueKeyFieldName = await GetKeyFieldNameViaApiAsync(sourceServiceName, sourceIndexName, sourceApiKey, apiVersion);

            // FIX: We must use the unique document key for both sorting AND filtering.
            string primarySortFieldName = uniqueKeyFieldName; 
            
            Console.WriteLine($"Starting migration of {totalDocuments:N0} documents...");
            Console.WriteLine($"Batch size: {batchSize}");
            Console.WriteLine($"Using unique key field for pagination: {primarySortFieldName}");
            Console.WriteLine(new string('=', 70));

            // Cursors are now just the unique key's value (which is both the sort key and the unique ID)
            string lastKey = null;
            ...
            while (hasMoreDocuments)
            {
                ...
                try
                {
                    ...
                    // We pass the unique key field name and the last unique key value
                    documents = await GetDocumentsWithCursorAsync(
                        sourceServiceName, sourceIndexName, sourceApiKey,
                        batchSize, primarySortFieldName, lastKey, apiVersion);
                    ...
                    if (batchSuccess)
                    {
                        ...
                        // CRITICAL: Advance the cursor ONLY on success
                        if (documents.Count > 0)
                        {
                            var lastDocument = documents.Last();
                            if (lastDocument.ContainsKey(primarySortFieldName))
                            {
                                // The value of the unique key field is the cursor for the next batch
                                lastKey = lastDocument[primarySortFieldName]?.ToString();
                            }
                        }
            ...
            if (missingCount != 0)
            {
                Console.WriteLine($"\n⚠ Warning: Migration completed because the source returned no more data. The discrepancy of {missingCount:N0} documents suggests the initial document count was inaccurate or documents were removed from the source index.");
            }
        }

        // NOTE: keyFieldName here is the UNIQUE key field (e.g., 'id')
            ...
            int top, string keyFieldName, string lastKey, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    // Filter: keyFieldName greater than the last key processed
                    string filter = null;
                    if (!string.IsNullOrEmpty(lastKey))
                    {
                        // Use 'gt' to get all documents strictly after the last one
                        filter = $"{keyFieldName} gt '{EscapeODataString(lastKey)}'";
                    }

                    // Order by the key field ascending for cursor pagination. 
                    // Since keyFieldName is the unique key, no secondary sort is needed.
