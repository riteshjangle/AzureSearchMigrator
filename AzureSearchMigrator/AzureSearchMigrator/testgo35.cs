    ...
    class Program
    {
           ...
           int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            try
            {
                ...
                // FIX: The main loop condition now primarily relies on hasMoreDocuments,
                // which is set to false when the source returns less than batchSize documents.
                // The totalMigratedDocuments check is less reliable if totalDocuments is slightly off.
            ...
            List<Dictionary<string, object>> documents, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        // The 'value' array now contains the flattened document objects
