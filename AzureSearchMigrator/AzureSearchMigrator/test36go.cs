    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                // Using index stats for initial count
        ...
        // Using index statistics for a more reliable document count
        static async Task<int> GetTotalDocumentCountViaApiAsync(string serviceName, string indexName, string apiKey, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    // Use the index statistics endpoint
            ...
            catch (Exception) { /* Swallow exception, return 0 below */ }
           ...
           int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            try
            {
                ...
                // FIX: Loop primarily relies on hasMoreDocuments, which is the reliable signal from the API.
                while (hasMoreDocuments)
                {
                    ...
                    // Safety check against infinite loops/excessive retries
                    if (batchNumber > (totalDocuments / batchSize) * 2 + 10 && totalMigratedDocuments < totalDocuments)
                    {
        ...
        // Corrected: Flattened the document structure and added @search.action directly
            ...
            List<Dictionary<string, object>> documents, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    // contains the document fields PLUS the required @search.action property.
                    var indexOperations = documents.Select(doc =>
                    {
                        // Create a new dictionary to hold the document + the action
                        var operation = new Dictionary<string, object>(doc)
                        {
                            // Add the required action property with the correct key name
