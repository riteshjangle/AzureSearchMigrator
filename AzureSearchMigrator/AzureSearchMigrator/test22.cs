    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                // Step 4: Migrate all data using cursor-based pagination
                Console.WriteLine("Step 4: Migrating data using cursor-based pagination...");
                await MigrateAllDocumentsWithCursorAsync(
        ...
        static async Task MigrateAllDocumentsWithCursorAsync(
            ...
            int batchSize, int totalDocuments, string apiVersion)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Using key field for cursor pagination: {keyFieldName}");
                ...
                while (hasMoreDocuments && totalMigratedDocuments < totalDocuments)
                {
                    ...
                    try
                    {
                        ...
                        if (documents.Count > 0)
                        {
                            ...
                            if (lastDocument.ContainsKey(keyFieldName) && lastDocument[keyFieldName] != null)
                            {
            ...
            int top, string keyFieldName, string lastKey, string apiVersion)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        // First batch - get the first set of documents
                        url = $"https://{serviceName}.search.windows.net/indexes/{indexName}/docs?api-version={apiVersion}" +
                              $"&$top={top}&$orderby={Uri.EscapeDataString(keyFieldName)} asc&search=*";
                    }
                    else
                    {
                        // Subsequent batches - use filter to get documents after the last key
                        string filter = $"{keyFieldName} gt '{EscapeODataString(lastKey)}'";
                        url = $"https://{serviceName}.search.windows.net/indexes/{indexName}/docs?api-version={apiVersion}" +
                              $"&$top={top}&$orderby={Uri.EscapeDataString(keyFieldName)} asc&$filter={Uri.EscapeDataString(filter)}";
                    }
