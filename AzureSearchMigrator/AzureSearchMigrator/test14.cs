    ...
    class Program
    {
            ...
            int totalDocuments)
        {
            ...
            try
            {
                ...
                // Use continuation tokens for proper pagination
                ...
                do
                {
                    ...
                    // Execute search with continuation token
                    SearchResults<SearchDocument> results;

                    if (continuationToken != null)
                    {
                        // For subsequent pages, we need to use a different approach
                        // Let's use skip and take for simple pagination
                        options.Skip = (batchNumber - 1) * batchSize;
                    }

                    try
                    {
                        results = await sourceClient.SearchAsync<SearchDocument>("*", options);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (batch.Count == 0)
                    {
                        Console.WriteLine($"No more documents found. Completed at batch {batchNumber - 1}");
                        break;
                    }
                ...
                } while (totalMigratedDocuments < totalDocuments && batchNumber < 500); // Safety limit
                ...
                if (totalMigratedDocuments < totalDocuments)
                {
                    ...
                    Console.WriteLine("This might be due to pagination issues or document count mismatch.");
                }
            }
            catch (Exception ex)
            {
