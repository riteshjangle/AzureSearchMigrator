...
namespace AzureSearchMigrator
{
    class Program
    {
            ...
            try
            {
                ...
                await foreach (var page in resultsAsync.AsPages())
                {
                    ...
                    while (!success && retryCount < maxRetries)
                    {
                        try
                        {
                            // Index the batch and access Results properly
                ...
                if (totalMigratedDocuments < totalDocuments)
                {
                    ...
                    int additionalDocuments = await MigrateMissingDocumentsAsync(sourceClient, targetClient, totalDocuments, totalMigratedDocuments);
                    totalMigratedDocuments += additionalDocuments;
                }
        ...
        static async Task<int> MigrateMissingDocumentsAsync(
            SearchClient sourceClient,
            SearchClient targetClient,
            int totalDocuments,
            int alreadyMigrated)
            ...
            try
            {
                ...
                foreach (var pageSize in strategies)
                {
                    ...
                    await foreach (var page in resultsAsync.AsPages())
                    {
                        ...
                        try
                        {
                            // Access IndexDocumentsResult properly
                            IndexDocumentsResult indexResult = await targetClient.IndexDocumentsAsync(IndexDocumentsBatch.Upload(batch));
